...
const rawRequestText = ref('')
const lastSentRawMap = ref<Record<string, string>>({})
const repeaterResponseMap = ref<Record<string, RowResponseState>>({})
...
const filteredTableRows = computed<RequestRow[]>(() => {
  const q = tableSearch.value.trim().toLowerCase()
  if (!q) return sortedTableRows.value
  return sortedTableRows.value.filter(r =>
...
const listTotal = computed(() => filteredTableRows.value.length)
...
const pagedTableRows = computed<RequestRow[]>(() => {
  ...
  return filteredTableRows.value.slice(start, start + size)
})
...
watch(() => tableSearch.value, () => { listPage.value = 1 })
...
function stripQueryFromRawRequest(raw: string): string {
  if (!raw) return raw
  const nl = raw.indexOf('\n')
  const firstLine = nl >= 0 ? raw.slice(0, nl) : raw
  const rest = nl >= 0 ? raw.slice(nl) : ''
  // GET /path?foo=bar HTTP/1.1  →  GET /path HTTP/1.1
  const stripped = firstLine.replace(/^(\S+\s+[^?\s]+)\?[^\s]*(\s+\S+)/, '$1$2')
  return stripped + rest
}

function buildRawResponseFromState(state: RowResponseState | null | undefined): string {
  const detail = state?.responseDetail
  if (!detail) return ''

  const code = detail.status_code || 0
  const statusText = HTTP_STATUS_TEXT[code] || ''
  const lines: string[] = [`HTTP/1.1 ${code} ${statusText}`]
  ...
  const body = String(detail.response_text || '')
  if (body) lines.push(body)
  else if (detail.error) lines.push(`[Error] ${detail.error}`)
  return lines.join('\n')
}

function buildRawResponse(row: RequestRow): string {
  return buildRawResponseFromState(rowResponse(row.key))
}
...
function buildRawFromDraft(draft: RowEditDraft): string {
  const method = draft.method
  const path = method === 'GET' && draft.queryText
    ? `${draft.path}?${draft.queryText}`
    : draft.path

  let host = ''
  try { host = new URL(draft.baseurl).host } catch {}

  let parsedHeaders: Record<string, string> = {}
  const hText = draft.headersText.trim()
  if (hText.startsWith('{')) {
    try { parsedHeaders = JSON.parse(hText) } catch {}
  } else {
    hText.split(/\r?\n/).forEach((line) => {
  ...
  if (method === 'POST' && draft.bodyText.trim()) {
    lines.push(draft.bodyText.trim())
  }
  return lines.join('\n')
}


function onOpenRowRepeater(row: RequestRow) {
  expandedRowKey.value = row.key
  onStartRowEdit(row)
  rawRequestText.value = editingDraft.value ? buildRawFromDraft(editingDraft.value) : ''
}
...
function parseRawRequest(raw: string) {
  ...
  const fullPath = firstParts[1] || '/'
  const parts = splitPathWithSuffix(fullPath)
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const k = line.slice(0, colonAt).trim()
    const v = line.slice(colonAt + 1).trim()
    if (k.toLowerCase() === 'host') { host = v } else { headers[k] = v }
  }
  const body = lines.slice(bodyStartIdx).join('\n')
  const headersText = Object.entries(headers).map(([k, v]) => `${k}: ${v}`).join('\n')
  const baseurl = host ? `https://${host}` : ''
  return { method, path: parts.main || fullPath, queryText: parts.suffix.replace(/^[?#]+/, ''), baseurl, headersText, body }
}

async function onSendRawRepeater(row: RequestRow) {
  if (!editingDraft.value || editingRowKey.value !== row.key) return
  if (!currentDomain.value) { error.value = '项目域名缺失'; return }
  if (row.endpoint_id <= 0) { error.value = '当前行未匹配到接口 ID'; return }

  const parsed = parseRawRequest(rawRequestText.value)
  editingDraft.value.method = normalizeSelectableMethod(parsed.method)
  editingDraft.value.path = parsed.path
  editingDraft.value.queryText = parsed.queryText
  if (parsed.baseurl) editingDraft.value.baseurl = parsed.baseurl
  editingDraft.value.headersText = parsed.headersText
  editingDraft.value.bodyText = parsed.body

  const draft = editingDraft.value
  const preparedBody = prepareRequestBody(draft.bodyText, draft.method, requestBodyType.value)
  if (!preparedBody.ok) { error.value = preparedBody.error; return }

  const requestUrlOverride = rowEditorPreviewUrl()
  if (!requestUrlOverride) { error.value = '临时请求 URL 为空'; return }
  ...
  try {
    ...
    lastSentRawMap.value = { ...lastSentRawMap.value, [row.key]: rawRequestText.value }
    repeaterResponseMap.value = { ...repeaterResponseMap.value, [row.key]: responseMap.value[row.key]! }
    persistRequestTabState()
  } catch (err) {
    error.value = resolveError(err)
  } finally {
...
<template>
  <section class="project-request-tab">
    ...
    <div class="sub-block">
      <div class="run-snapshot-strip">
        <template v-if="runSnapshots.length">
        ...
        <div class="snapshot-search">
          <input
            ...
            type="text"
            placeholder="搜索接口"
            class="snapshot-search-input"
          />
          <span v-if="tableSearch" class="table-search-count">{{ filteredTableRows.length }}/{{ tableRows.length }}</span>
          <button v-if="tableSearch" class="ghost btn-sm" @click="tableSearch = ''">×</button>
        </div>
      </div>
        ...
        <tbody>
          <template v-for="(row, index) in pagedTableRows" :key="row.key">
            ...
            >
              ...
              <td class="mono url-col">{{ row.url ? stripUrlQuery(row.url) : '-' }}</td>
              ...
              <td class="action-col">
                <button
                  ...
                  @click.stop="onOpenRowRepeater(row)"
                >
                  {{ rowSending(row.key) ? '发送中...' : '重放' }}
                </button>
              </td>
            </tr>
            ...
            >
              <td colspan="8">
                <div class="response-wrap" :class="{ 'response-wrap-repeater': editorActiveFor(row.key) }">

                  <!-- 编辑模式：左右分栏 Repeater -->
                  <template v-if="editorActiveFor(row.key) && editingDraft">
                    <div class="rp-left repeater-pane">
                      <div class="repeater-pane-head">
                        <span class="repeater-pane-label">Request</span>
                        <div class="rp-actions">
                          <button class="ghost btn-sm" @click.stop="onCancelRowEdit">取消</button>
                          <button
                            class="ghost btn-sm"
                            :disabled="rowSending(row.key) || row.endpoint_id <= 0"
                            @click.stop="onSendRawRepeater(row)"
                          >
                            {{ rowSending(row.key) ? '发送中..' : '发送' }}
                          </button>
                        </div>
                      </div>
                      <textarea class="rp-raw-textarea" v-model="rawRequestText" spellcheck="false"></textarea>
                    </div>

                    <div class="rp-right repeater-pane">
                      <div class="repeater-pane-head">
                        <span class="repeater-pane-label">Response</span>
                        <div v-if="repeaterResponseMap[row.key]?.responseDetail?.response_truncated" class="truncate-notice">
                          响应体过大（原始 {{ Math.round((repeaterResponseMap[row.key]?.responseDetail?.response_original_length ?? 0) / 1024) }} KB），仅展示前 500KB
                        </div>
                      </div>
                      <div v-if="rowSending(row.key)" class="empty detail-loading">发送中...</div>
                      <pre v-else class="repeater-pre rp-response-pre">{{ buildRawResponseFromState(repeaterResponseMap[row.key]) || 'No response.' }}</pre>
                    </div>
                  </template>

                  <!-- 查看模式：上下 Repeater -->
                  <template v-else>
                    <div v-if="rowResponse(row.key)" class="repeater-wrap">
                      <div v-if="lastSentRawMap[row.key]" class="repeater-pane">
                        <div class="repeater-pane-label">Request</div>
                        <pre class="repeater-pre">{{ stripQueryFromRawRequest(lastSentRawMap[row.key] ?? '') }}</pre>
                      </div>
                      <div class="repeater-pane">
                        <div class="repeater-pane-head">
                          <span class="repeater-pane-label">Response</span>
                          <div v-if="rowResponse(row.key)?.responseDetail?.response_truncated" class="truncate-notice">
                            响应体过大（原始 {{ Math.round((rowResponse(row.key)?.responseDetail?.response_original_length ?? 0) / 1024) }} KB），仅展示前 500KB
                          </div>
                        </div>
                        <div v-if="rowResponseDetailLoading(row.key)" class="empty detail-loading">loading...</div>
                        <pre v-else class="repeater-pre">{{ buildRawResponse(row) || 'empty body' }}</pre>
                      </div>
                    </div>
...
.truncate-notice {
  ...
  margin-bottom: 4px;
  padding: 3px 8px;
  background: var(--bg-warn, #fffbe6);
  border-left: 3px solid var(--warn, #f0a500);
  border-radius: 3px;
}
.project-request-tab {
  display: grid;
...
.repeater-pane-head .repeater-pane-label {
  padding: 0;
  background: transparent;
  border-bottom: none;
}

.repeater-pane-head .truncate-notice {
  margin-bottom: 0;
  flex: 1;
}
