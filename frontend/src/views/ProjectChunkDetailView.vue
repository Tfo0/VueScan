...
import ProjectApiLlmTab from '../components/project/ProjectApiLlmTab.vue'
...
const activeView = ref<'route' | 'js' | 'requestCapture' | 'requestMap' | 'analysis' | 'apiExtract' | 'apiLlm' | 'baseRequest' | 'apiRequest'>('analysis')
...
// 概览摘要
const summaryRouteCount = computed(() => Number(project.value?.route_count ?? detail.value?.route_count ?? 0))
const summaryJsCount = computed(() => Number(project.value?.js_count ?? detail.value?.js_count ?? 0))
const summaryLocateApi = computed(() => String(autoPipelineTarget.value.request_url || '').trim())
const summaryLocateFile = computed(() => {
  const file = String(autoPipelineTarget.value.file_name || '').trim()
  const line = String(autoPipelineTarget.value.line || '').trim()
  return file ? (line ? `${file}:${line}` : file) : ''
})
const summaryPattern = computed(() => String(autoRegexResult.value.selected_pattern || '').trim())
const summaryBaseUrl = computed(() => String(autoInferResult.value.baseurl || '').trim())
const summaryBaseApi = computed(() => String(autoInferResult.value.baseapi || '').trim())
const summaryBestResult = computed(() => project.value?.best_request_result ?? null)
      ...
      <div v-else-if="detail" class="detail-wrap">
        <div class="project-toolbar">
          <div class="toolbar-tabs">
            ...
            <a href="#" class="nav-tab" :class="{ active: activeView === 'apiLlm' }" @click.prevent="activeView = 'apiLlm'">
              <span>LLM分析</span>
            </a>
        ...
        <section v-if="liveJobCard && activeView === 'js'" class="job-live-card">
    ...
    <section class="panel">
      ...
      <template v-else-if="activeView === 'analysis'">
        ...
        <div class="db-wrap">
          ...
          <!-- 顶部：域名 + 优先级 + 原因 -->
          <div class="db-header" :class="`db-header--${analysisLevel || 'pending'}`">
            <div class="db-header-left">
              <div class="db-domain">{{ domain }}</div>
              <div v-if="project?.request_value_reason" class="db-reason">{{ project.request_value_reason }}</div>
            </div>
            <span class="analysis-priority db-priority" :class="analysisLevel || 'pending'">{{ analysisPriorityText }}</span>
          </div>

          <!-- 统计卡片行 -->
          <div class="db-stats">
            <div class="db-stat">
              <div class="db-stat-n">{{ summaryRouteCount }}</div>
              <div class="db-stat-l">路由</div>
            </div>
            <div class="db-stat">
              <div class="db-stat-n">{{ summaryJsCount }}</div>
              <div class="db-stat-l">JS 文件</div>
            </div>
            <div class="db-stat">
              <div class="db-stat-n">{{ chunkLocalCount }}<span class="db-stat-total">/{{ chunkTotalCount }}</span></div>
              <div class="db-stat-l">已下载 Chunk</div>
            </div>
            <div class="db-stat">
              <div class="db-stat-n">{{ project?.request_value_snapshot_count || 0 }}</div>
              <div class="db-stat-l">请求快照</div>
            </div>
            <div class="db-stat">
              <div class="db-stat-n">{{ project?.saved_result_count || 0 }}</div>
              <div class="db-stat-l">已保存结果</div>
            </div>
          </div>
          ...
          <div class="db-grid">
            ...
            <!-- 左：扫描配置 -->
            <div class="db-card">
              <div class="db-card-head">扫描配置</div>
              <table class="db-table">
                <tbody>
                  <tr v-if="summaryBaseUrl">
                    <td class="db-key">BaseURL</td>
                    <td class="mono">{{ summaryBaseUrl }}</td>
                  </tr>
                  <tr v-if="summaryBaseApi">
                    <td class="db-key">BaseAPI</td>
                    <td class="mono">{{ summaryBaseApi }}</td>
                  </tr>
                  <tr v-if="summaryPattern">
                    <td class="db-key">正则</td>
                    <td class="mono db-break">{{ summaryPattern }}</td>
                  </tr>
                  <tr v-if="summaryLocateFile">
                    <td class="db-key">定位文件</td>
                    <td class="mono db-break">{{ summaryLocateFile }}</td>
                  </tr>
                  <tr v-if="summaryLocateApi">
                    <td class="db-key">JS 定位</td>
                    <td class="mono db-break">{{ summaryLocateApi }}</td>
                  </tr>
                  <tr v-if="!summaryBaseUrl && !summaryBaseApi && !summaryPattern && !summaryLocateFile && !summaryLocateApi">
                    <td colspan="2" class="db-empty-cell">自动化流程尚未完成，暂无配置数据</td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <!-- 右：自动化结果 -->
            <div class="db-card">
              <div class="db-card-head">最佳响应</div>
              <template v-if="summaryBestResult">
                <div class="db-best">
                  <span class="summary-method">{{ summaryBestResult.method }}</span>
                  <span class="mono db-best-path">{{ summaryBestResult.path }}</span>
                </div>
                <div class="db-best-meta">
                  <span class="summary-status ok">{{ summaryBestResult.status_code }}</span>
                  <span class="db-best-size">{{ Math.round(summaryBestResult.response_length / 1024) }} KB</span>
                </div>
              </template>
              <div v-else class="db-empty-cell">暂无有效响应数据</div>

              <div class="db-card-head db-card-head--mt">LLM 安全分析</div>
              <div class="db-llm-hint">
                切换到顶部 <strong>LLM 分析</strong> 标签查看漏洞、未授权建议及接口攻击思路
              </div>
            </div>
      ...
      <div v-show="activeView === 'apiLlm'">
        <ProjectApiLlmTab :domain="domain" />
      </div>

      <template v-if="activeView === 'apiExtract'">
      ...
      <template v-if="activeView === 'baseRequest'">
      ...
      <template v-if="activeView === 'apiRequest'">
...
.db-header--high { background: #fff5f5; border-color: #f5c6c6; }
.db-header--medium { background: #fffbf0; border-color: #f0dea0; }
.db-header--low { background: #f0faf4; border-color: #b0dfc0; }

.db-header-left { display: flex; flex-direction: column; gap: 6px; min-width: 0; }
.db-domain { font-size: 16px; font-weight: 700; color: #0d2d4a; word-break: break-all; }
.db-reason { font-size: 13px; color: #4a6478; line-height: 1.5; }
.db-priority { font-size: 13px; white-space: nowrap; flex-shrink: 0; }
