...
from src.vue_api.llm_analyzer import analyze_api_paths_with_llm as _module_analyze_api_paths_with_llm
from src.vue_api.project_store import save_llm_analysis_result as _module_save_llm_analysis_result
from src.vue_api.project_store import load_llm_analysis_result as _module_load_llm_analysis_result
from src.vue_api.param_store import (
    extract_and_store_response_params as _module_extract_response_params,
    load_param_keys as _module_load_param_keys,
)
...
from src.vue_chunk.job_state import (
    ...
    collect_sync_status_map as _module_collect_sync_status_map,
...
def _migrate_project_stats_once() -> None:
    """启动时一次性把已有项目的路由数和 JS 数写入 projects 表（只补 -1 的行）。"""
    import json as _json
    import threading
    from pathlib import Path as _Path
    from src.web.state_store import list_projects, update_project_stats
    ...
    def _run() -> None:
        try:
            ...
            for record in records:
                ...
                if not domain:
                    continue
                rc = int(record.get("route_count", -1) if record.get("route_count") is not None else -1)
                jc = int(record.get("js_count", -1) if record.get("js_count") is not None else -1)
                if rc >= 0 and jc >= 0:
                    continue  # 已有数据，跳过
                base = _Path(str(BOOTSTRAP_CONFIG.projects_dir)) / domain
                new_rc, new_jc = rc, jc
                if rc < 0:
                    routes_file = base / "vueRouter" / "routes.json"
                    if routes_file.is_file():
                        try:
                            payload = _json.loads(routes_file.read_text(encoding="utf-8"))
                            routes = payload.get("routes", []) if isinstance(payload, dict) else []
                            new_rc = len([r for r in routes if isinstance(r, dict)])
                        except Exception:
                            pass
                if jc < 0:
                    js_file = base / "vueRouter" / "js.txt"
                    if js_file.is_file():
                        try:
                            new_jc = len([ln for ln in js_file.read_text(encoding="utf-8").splitlines() if ln.strip()])
                        except Exception:
                            pass
                if new_rc != rc or new_jc != jc:
                    update_project_stats(domain, route_count=new_rc, js_count=new_jc)
        except Exception:
            pass
...
def _run_api_llm_analyze(
    *,
    ...
    return _module_analyze_api_paths_with_llm(
        paths=paths,
        settings=settings,
        safe_str=_safe_str,
...
def _run_llm_analyze_background(*, job_id: str, domain: str, paths: list[str] | None) -> None:
    try:
        ...
        result = _run_api_llm_analyze(
            ...
            progress_callback=lambda msg: append_log(job_id, msg),
        )
        _module_save_llm_analysis_result(domain, result)
        append_log(job_id, f"LLM 分析完成，接口数: {len(result.get('api_analysis') or [])}")
        update_job(job_id, status="completed", result=result)
    except Exception as exc:
        append_log(job_id, f"LLM 分析失败: {exc}")
        update_job(job_id, status="failed", error=str(exc))


def _start_llm_analyze_job(domain: str, paths: list[str] | None) -> str:
    # 把同 domain 的旧 running job 标为 failed，避免僵尸 job 占用轮询
    from src.services.job_store import iter_job_payloads, update_job as _update_job
    for old in iter_job_payloads(limit=20, step="vueapi_llm_analyze", with_logs=False):
        if old.get("status") == "running" and (old.get("payload") or {}).get("domain") == domain:
            _update_job(old["job_id"], status="failed", error="被新任务取代")
    job = create_job(step="vueapi_llm_analyze", payload={"domain": domain})
    job_id = job["job_id"]
    from src.web.runtime import spawn_background
    spawn_background(
        _run_llm_analyze_background,
        job_id=job_id,
        domain=domain,
        paths=paths,
    )
...
APP_ROUTE_WIRING = _build_web_app_routes(
    _WebAppRouteWiringDeps(
        ...
        collect_sync_status_map=_module_collect_sync_status_map,
        ...
        run_api_llm_analyze=RUN_API_LLM_ANALYZE,
        save_llm_analysis_result=_module_save_llm_analysis_result,
        load_llm_analysis_result=_module_load_llm_analysis_result,
        start_llm_analyze_job=_start_llm_analyze_job,
    )
)
