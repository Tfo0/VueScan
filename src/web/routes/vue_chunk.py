...
class VueChunkRouteDeps:
    ...
    collect_sync_status_map: Callable[[], dict[str, str]]
...
async def _api_vue_chunk_projects(request: Request, deps: VueChunkRouteDeps):
    ...
    project_records, sync_status_map = await asyncio.gather(
        asyncio.to_thread(deps.list_projects, limit=3000),
        asyncio.to_thread(deps.collect_sync_status_map),
    )
    ...
    for domain in project_domains:
        ...
        project_item["last_activity_at"] = deps.safe_str(
            project_item.get("updated_at") or project_item.get("created_at")
        )
        project_item["sync_status"] = deps.normalize_sync_status(
            sync_status_map.get(domain, "idle")
        )
        project_item["sync_job_id"] = ""
...
async def _api_vue_chunk_project_retry(request: Request, deps: VueChunkRouteDeps):
    ...
    sync_map = await asyncio.to_thread(deps.collect_sync_state_map, limit=1200)
...
async def _api_vue_chunk_project_detail(request: Request, deps: VueChunkRouteDeps):
    ...
    detail, sync_map, js_download_map, request_capture_map = await asyncio.gather(
        asyncio.to_thread(
            deps.load_project_detail,
            domain,
        ...
        asyncio.to_thread(deps.collect_sync_state_map, limit=1200),
        asyncio.to_thread(deps.collect_js_download_state_map, limit=1200),
        asyncio.to_thread(deps.collect_request_capture_state_map, limit=1200),
    )
    if not detail:
        return deps.json_error(f"project not found: {domain}", status_code=404)
