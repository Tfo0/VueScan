...
class VueApiRouteDeps:
    ...
    run_api_llm_analyze: Callable[..., dict[str, Any]]
    save_llm_analysis_result: Callable[[str, dict[str, Any]], str]
    load_llm_analysis_result: Callable[[str], dict[str, Any]]
    start_llm_analyze_job: Callable[[str, Any], str]
...
async def _api_vue_api_context(request: Request, deps: VueApiRouteDeps):
    ...
    llm_analysis = deps.load_llm_analysis_result(selected_domain) if selected_domain else {}
...
async def _api_vue_api_llm_analyze(request: Request, deps: VueApiRouteDeps):
    payload = await _read_json_payload(request)
    domain = deps.safe_str(payload.get("domain"))
    if not domain:
        return deps.json_error("domain is required", status_code=400)
    ...
    if isinstance(raw_paths, list):
        manual_paths = [deps.safe_str(p) for p in raw_paths if deps.safe_str(p)]

    try:
        job_id = deps.start_llm_analyze_job(domain, manual_paths)
        return deps.json_ok({"job_id": job_id})
    except Exception as exc:
        return deps.json_error(str(exc), status_code=500)


async def _api_vue_api_llm_job_poll(request: Request, deps: VueApiRouteDeps):
    job_id = request.path_params.get("job_id", "")
    if not job_id:
        return deps.json_error("job_id required", status_code=400)
    try:
        job = deps.read_job(job_id, stale_minutes=20)
    except FileNotFoundError:
        return deps.json_error("job not found", status_code=404)
    except Exception as exc:
        return deps.json_error(str(exc), status_code=500)
    return deps.json_ok({
        "job_id": job["job_id"],
...
async def _api_vue_api_llm_save(request: Request, deps: VueApiRouteDeps):
    payload = await _read_json_payload(request)
    domain = deps.safe_str(payload.get("domain"))
    ...
    if not isinstance(result, dict):
        return deps.json_error("result is required", status_code=400)
    try:
        output_path = deps.save_llm_analysis_result(domain, result)
        return deps.json_ok({"saved": True, "output_path": output_path})
    except Exception as exc:
        return deps.json_error(str(exc), status_code=500)
...
def build_vue_api_routes(deps: VueApiRouteDeps) -> list[Route]:
    ...
    return [
        ...
        Route("/api/vueApi/llm-analyze", endpoint=_bind_route(_api_vue_api_llm_analyze, deps), methods=["POST"]),
        Route("/api/vueApi/llm-jobs/{job_id}", endpoint=_bind_route(_api_vue_api_llm_job_poll, deps), methods=["GET"]),
        Route("/api/vueApi/llm-save", endpoint=_bind_route(_api_vue_api_llm_save, deps), methods=["POST"]),
    ]
