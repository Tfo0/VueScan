...
def serialize_project(item: dict[str, Any], *, safe_str) -> dict[str, Any]:
    ...
    return {
        ...
        "route_count": int(item["route_count"]) if isinstance(item.get("route_count"), int) else -1,
        "js_count": int(item["js_count"]) if isinstance(item.get("js_count"), int) else -1,
        "pipeline_status": str(item.get("pipeline_status") or ""),
    }
