...
def _normalize_project_record(raw: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "route_count": int(raw.get("route_count", -1) if raw.get("route_count") is not None else -1),
        "js_count": int(raw.get("js_count", -1) if raw.get("js_count") is not None else -1),
...
def _project_records_from_db() -> list[dict[str, Any]]:
    ...
    with connect_web_sqlite(SQLITE_DB_FILE) as connection:
        rows = connection.execute(
            """
            SELECT domain, title, source, seed_urls_json, task_ids_json,
                   created_at, updated_at,
                   COALESCE(route_count, -1) as route_count,
                   COALESCE(js_count, -1) as js_count,
                   COALESCE(pipeline_status, '') as pipeline_status
            FROM projects
...
def update_project_stats(domain: str, *, route_count: int, js_count: int) -> None:
    """更新项目路由数和 JS 数；传 -1 表示跳过该字段。"""
    token = str(domain or "").strip()
    if not token:
        return
    ensure_web_sqlite_schema(SQLITE_DB_FILE)
    with connect_web_sqlite(SQLITE_DB_FILE) as connection:
        if route_count >= 0 and js_count >= 0:
            connection.execute(
                "UPDATE projects SET route_count = ?, js_count = ? WHERE domain = ?",
                (int(route_count), int(js_count), token),
            )
        elif route_count >= 0:
            connection.execute(
                "UPDATE projects SET route_count = ? WHERE domain = ?",
                (int(route_count), token),
            )
        elif js_count >= 0:
            connection.execute(
                "UPDATE projects SET js_count = ? WHERE domain = ?",
                (int(js_count), token),
            )
        connection.commit()


def update_pipeline_status(domain: str, status: str) -> None:
    token = str(domain or "").strip()
    if not token:
        return
    ensure_web_sqlite_schema(SQLITE_DB_FILE)
    with connect_web_sqlite(SQLITE_DB_FILE) as connection:
        connection.execute(
            "UPDATE projects SET pipeline_status = ? WHERE domain = ?",
            (str(status or "").strip(), token),
        )
        connection.commit()
