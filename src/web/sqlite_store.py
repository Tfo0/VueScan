...
def _migrate_projects_columns(connection: sqlite3.Connection) -> None:
    existing = {row[1] for row in connection.execute("PRAGMA table_info(projects)").fetchall()}
    if "route_count" not in existing:
        connection.execute("ALTER TABLE projects ADD COLUMN route_count INTEGER NOT NULL DEFAULT -1")
    if "js_count" not in existing:
        connection.execute("ALTER TABLE projects ADD COLUMN js_count INTEGER NOT NULL DEFAULT -1")
    if "pipeline_status" not in existing:
        connection.execute("ALTER TABLE projects ADD COLUMN pipeline_status TEXT NOT NULL DEFAULT ''")
...
def ensure_web_sqlite_schema(database_file: Path) -> None:
    ...
    with connect_web_sqlite(database_file) as connection:
        ...
        _migrate_projects_columns(connection)
        connection.executescript(
            """
            CREATE TABLE IF NOT EXISTS api_response_params (
                ...
                updated_at TEXT NOT NULL,
                UNIQUE(domain, endpoint_path, method, field_key)
            );

            CREATE INDEX IF NOT EXISTS idx_api_response_params_domain
            ON api_response_params(domain);
            """
        )
