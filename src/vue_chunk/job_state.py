...
def _iter_latest_job_payloads(jobs_dir: Path, *, limit: int, step: str) -> list[dict[str, Any]]:
    try:
        # 状态图只需要 job 元数据和 result，不需要 logs，跳过 N+1 查询
        rows = iter_job_payloads(limit=max(1, int(limit)), step=step, with_logs=False)
    except Exception:
        rows = []
    if rows:
        return rows
...
def collect_sync_status_map() -> dict[str, str]:
    """列表页专用：一次 SQL 查出所有域名的最新 sync 状态，不扫文件不加载 logs。"""
    from src.services.job_store import iter_job_payloads, JOBS_DIR
    from src.services.job_store_db import JOB_DB_FILE, connect_job_store, ensure_job_store_schema

    SYNC_STEPS = {
        "web_module2_project_sync",
        "web_module2_js_download",
        "web_module2_request_capture",
    }
    ensure_job_store_schema(JOB_DB_FILE)
    result: dict[str, str] = {}
    try:
        with connect_job_store(JOB_DB_FILE) as conn:
            rows = conn.execute(
                """
                SELECT json_extract(payload_json, '$.domain') as domain,
                       json_extract(result_json, '$.domain') as domain2,
                       status, updated_at
                FROM jobs
                WHERE step IN ('web_module2_project_sync',
                               'web_module2_js_download',
                               'web_module2_request_capture')
        ...
        for row in rows:
            domain = _safe_text(row["domain"] or row["domain2"])
            if not domain:
                continue
            # 只保留每个域名最新的一条（已按 updated_at DESC 排序）
            if domain not in result:
                result[domain] = _safe_text(row["status"])
    except Exception:
        pass
    return result
