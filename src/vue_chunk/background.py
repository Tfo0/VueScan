...
def _derive_pipeline_status(result: dict[str, Any]) -> str:
    """从 sync job result 推导最终流程状态标签。"""
    def _safe(d: Any, *keys: str) -> str:
    ...
    # 路由
    if _int(result.get("url_count")) == 0:
        return "路由为空"

    # 没有开启 auto_pipeline → 只是基础同步
    if not result.get("auto_pipeline"):
        return "已同步"

    # 定位 JS
    target = result.get("auto_pipeline_target") or {}
    if not isinstance(target, dict) or not target.get("selected"):
        return "定位失败"

    # 正则
    regex = result.get("auto_regex") or {}
    if not isinstance(regex, dict) or not _safe(regex, "selected_pattern"):
        return "正则失败"

    # 基址推断
    infer = result.get("infer_result") or {}
    if not isinstance(infer, dict) or not infer.get("inferred"):
        return "推断失败"

    # 请求结果
    auto_req = result.get("auto_request") or {}
    get_snap = (auto_req.get("get_snapshot") or {}) if isinstance(auto_req, dict) else {}
    post_snap = (auto_req.get("post_snapshot") or {}) if isinstance(auto_req, dict) else {}
    if not _safe(get_snap, "snapshot_id") and not _safe(post_snap, "snapshot_id"):
        return "请求未完成"

    # 价值评级（从 request 结果里拿）
    get_summary = (auto_req.get("get_summary") or {}) if isinstance(auto_req, dict) else {}
    ok_count = _int(get_summary.get("ok", 0)) + _int(
        (auto_req.get("post_summary") or {}).get("ok", 0) if isinstance(auto_req, dict) else 0
    )
    total_count = _int(get_summary.get("total", 0)) + _int(
        (auto_req.get("post_summary") or {}).get("total", 0) if isinstance(auto_req, dict) else 0
    )
    if total_count > 0:
...
def run_module2_project_sync_background(
    *,
    ...
    auto_llm: bool = False,
    run_auto_llm_analyze: Callable[[str], dict[str, Any]] | None = None,
    extract_response_params: Callable[[str], dict[str, Any]] | None = None,
) -> None:
    ...
    try:
        ...
        # sync 完成后把路由数和 JS 数写入 projects 表，列表页直接读不用重算
        try:
            from config import PROJECTS_DIR
            from pathlib import Path as _Path
            import json as _json
            _sync_domain = safe_str(result.get("domain")) or domain
            if _sync_domain:
                _routes_file = _Path(str(PROJECTS_DIR)) / _sync_domain / "vueRouter" / "routes.json"
                _js_file = _Path(str(PROJECTS_DIR)) / _sync_domain / "vueRouter" / "js.txt"
                _route_count = 0
                _js_count = 0
                if _routes_file.is_file():
                    try:
                        _payload = _json.loads(_routes_file.read_text(encoding="utf-8"))
                        _routes = _payload.get("routes", []) if isinstance(_payload, dict) else []
                        _route_count = len([r for r in _routes if isinstance(r, dict)])
                    except Exception:
                        pass
                if _js_file.is_file():
                    try:
                        _js_count = len([ln for ln in _js_file.read_text(encoding="utf-8").splitlines() if ln.strip()])
                    except Exception:
                        pass
                from src.web.state_store import update_project_stats
                update_project_stats(_sync_domain, route_count=_route_count, js_count=_js_count)
        except Exception:
            pass
        ...
        if detect_request:
            ...
            if auto_scan_enabled:
                ...
                if bool(infer_result.get("inferred")):
                    ...
                    if extract_response_params is not None:
                        _checkpoint("extract_params")
                        append_log(job_id, "response param extraction start")
                        try:
                            param_result = extract_response_params(domain)
                            result["param_extract"] = param_result
                            append_log(job_id, f"response param extraction done: {param_result.get('count', 0)} fields")
                        except Exception as param_exc:
                            result["param_extract"] = {"error": str(param_exc)}
                            append_log(job_id, f"response param extraction failed: {param_exc}")
                else:
                    append_log(
                        job_id,
                ...
                if auto_llm and run_auto_llm_analyze is not None:
                    _checkpoint("llm_analyze")
                    append_log(job_id, "auto llm analyze start")
                    _update_phase("llm_analyze")
                    try:
                        llm_result = run_auto_llm_analyze(domain)
                        result["llm_result"] = llm_result if isinstance(llm_result, dict) else {}
                        append_log(job_id, "auto llm analyze completed")
                    except Exception as llm_exc:
                        result["llm_result"] = {"error": str(llm_exc)}
                        append_log(job_id, f"auto llm analyze failed: {llm_exc}")
        ...
        # pipeline 完成后写入最终状态标签
        try:
            _final_domain = safe_str(result.get("domain")) or domain
            if _final_domain:
                _ps = _derive_pipeline_status(result)
                from src.web.state_store import update_pipeline_status
                update_pipeline_status(_final_domain, _ps)
        except Exception:
            pass
    ...
    *,
    ...
    try:
        ...
        if stopped:
            ...
            append_log(job_id, "web action completed")
            update_job(job_id=job_id, status="completed", result=result)
            # capture 完成后更新 js_count（js.txt 已写好）
            try:
                from config import PROJECTS_DIR as _PDIR
                from pathlib import Path as _P
                _js_file2 = _P(str(_PDIR)) / domain / "vueRouter" / "js.txt"
                if _js_file2.is_file():
                    _js_count2 = len([ln for ln in _js_file2.read_text(encoding="utf-8").splitlines() if ln.strip()])
                    from src.web.state_store import update_project_stats
                    update_project_stats(domain, route_count=-1, js_count=_js_count2)
            except Exception:
                pass
    except Exception as exc:
        message = str(exc)
