...
from src.vue_api.saved_results import load_saved_request_results, find_best_request_result
...
def load_project_metrics(domain: str) -> dict[str, object]:
    ...
    try:
        saved_results = load_saved_request_results(value)
        saved_result_count = len(saved_results)
    except Exception:
        saved_result_count = 0
    try:
        best_request_result = find_best_request_result(value)
    except Exception:
        best_request_result = None
    try:
    ...
    result = {
        ...
        "best_request_result": best_request_result,
        **request_value,
    }
    try:
        from src.web.state_store import update_project_stats
        update_project_stats(value, route_count=route_count, js_count=js_count)
    except Exception:
        pass
    return result
