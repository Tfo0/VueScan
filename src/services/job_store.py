...
def _job_payload_from_row(connection, row, *, with_logs: bool = True) -> dict:
    if with_logs:
        logs = [
            {
...
def _is_stale_running(data: dict, max_minutes: int) -> bool:
    """检测是否是卡死的 running job：超过 max_minutes 没有任何状态更新。"""
...
def read_job(job_id: str, *, stale_minutes: int = 30) -> dict:
    """读取 job，如果发现 running 状态超过 stale_minutes 分钟没更新，自动标为 failed。"""
    with _JOB_IO_LOCK:
        data = _load_or_import_job(job_id)
        if _is_stale_running(data, stale_minutes):
            data["status"] = "failed"
            data["error"] = f"任务超时：已运行超过 {stale_minutes} 分钟无响应（线程可能已异常终止）"
            data["updated_at"] = utc_now_iso()
            data["finished_at"] = utc_now_iso()
            data = _save_job_payload(data)
        return data
...
def iter_job_payloads(limit: int = 30, *, step: str | None = None, with_logs: bool = True) -> list[dict]:
    with _JOB_IO_LOCK:
        ensure_job_store_schema()
        query = """
            SELECT job_id, step, status, created_at, updated_at, finished_at, payload_json, result_json, error_text
        ...
        with connect_job_store() as connection:
            ...
            if rows:
                return [_job_payload_from_row(connection, row, with_logs=with_logs) for row in rows]
