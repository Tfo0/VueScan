...
_BORING_PATH_KEYWORDS = frozenset([
    "captcha", "verify", "verification", "login", "logout", "signin", "signout",
    "ping", "health", "healthcheck", "favicon", "static", "public", "common",
    "refresh", "token", "csrf", "options", "actuator", "metrics",
    "sms", "email", "sendcode", "getcode", "checkcode", "validatecode",
])


def find_best_request_result(domain: str) -> dict[str, object] | None:
    ...
    for row in rows:
        ...
        if status_code != 200:
            continue
        response_length = int(row.get("response_length") or 0)
        if response_length < 200:
            continue
        path = str(row.get("path") or "").lower()
        if any(kw in path for kw in _BORING_PATH_KEYWORDS):
            continue
        candidates.append(row)
    if not candidates:
        return None
    best = max(candidates, key=lambda r: int(r.get("response_length") or 0))
    return {
        "method": str(best.get("request_method") or "GET"),
        "path": str(best.get("path") or ""),
        "status_code": int(best.get("status_code") or 0),
        "response_length": int(best.get("response_length") or 0),
        "endpoint_id": int(best.get("endpoint_id") or 0),
    }
