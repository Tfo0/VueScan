...
def load_saved_response_detail(response_path: object) -> dict[str, object]:
    ...
    if not isinstance(loaded, dict):
        return {}
    response_text = str(loaded.get("response_text") or "")
    if len(response_text) > _RESPONSE_TEXT_LIMIT:
        loaded = dict(loaded)
        loaded["response_text"] = response_text[:_RESPONSE_TEXT_LIMIT]
        loaded["response_truncated"] = True
        loaded["response_original_length"] = len(response_text)
    return loaded
