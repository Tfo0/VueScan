...
def _llm_analysis_file_path(domain: str) -> Path | None:
    token = _safe_text(domain)
    if not token:
        return None
    return PROJECTS_DIR / token / "vueApi" / "llm_analysis.json"


def save_llm_analysis_result(domain: str, result: dict[str, Any]) -> str:
    path = _llm_analysis_file_path(domain)
    if path is None:
...
def load_llm_analysis_result(domain: str) -> dict[str, Any]:
    path = _llm_analysis_file_path(domain)
